...
uchar intersec_lin_lin(const line3 *res_pt l1, line3 *res_pt l2, point3 *res_pt p, double *res_pt cangl)
{
	...
	cross3((*l1).r, (*l2).r, t1);
	if(len_squ3(t1) == 0.) return 0;
	sub3((*l2).o, (*l1).o, t2);
	if(dot3(t1, t2) != 0.) return 0;
	t3 = (*l1).r[1] / (*l1).r[0];
...
{
	double t1[3], t2, t3;
	if(fabs(t3 = dot3((*e).n, (*l).r)) < DBL_EPSILON) return 0;
	sub3((*e).o, (*l).o, t1);
	t2 = dot3((*e).n, t1);
	t2 /= t3;
	(*l).l = t2;
...
uchar intersec_lin_tria(const point3 *res_pt tria, const point3 *res_pt pin, line3 *res_pt l, const uchar ret, intrsec *res_pt is)
{
	...
	if(dist_poi_pla(pin, &pt) < 0.) rev3_ip(pt.n);
	if(fabs(t3 = dot3(pt.n, (*l).r)) < DBL_EPSILON) return 0;
	sub3(pt.o, (*l).o, t1);
	t2 = dot3(pt.n, t1) / t3;
	if(t2 < 0.) return 0;
	...
	else if(t4[1] > t4[2]) i = 1;
	else i = 2;
	if(t5[(i + 1) % 3] > t5[(i + 2) % 3]) j = (i + 1) % 3;
	else j = (i + 2) % 3;
	r1x = t4[i];
	r2x = t5[i];
	gox = (*l).o[i];
 ...
 * if there is no intersection or the multiplier is negativ, return 0, otherwise 1
 */
uchar intersec_lin_sph(const sphere3 *res_pt e, line3 *res_pt l, const uchar ret, intrsec *res_pt is)
{
	...
	if(ret)
	{
		...
		/**< attention here: */
		sub3((*e).o, (*is).p, t1); /**< normal vector points to center of sphere */
