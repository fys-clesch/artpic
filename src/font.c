 ...
 *
 */
void rasterfont(const uchar init)
{
	if(init)
	{
		uint i;
		...
		for(i = 32; i < 127; i++)
		{
			...
			glBitmap(8, 13, 0., 2., 10., 0., rasters[i - 32]);
			glEndList();
		}
	}
...
void printstring(const char *s)
{
	...
	glCallLists(strlen(s), GL_UNSIGNED_BYTE, (uchar *)s);
	glPopAttrib();
}
...
void fonttest(void)
{
	...
	glColor3d(1., 1., 1.);
	glRasterPos2d(-1.1, .9);
	glutBitmapString(GLUT_BITMAP_8_BY_13, (const uchar *)"These keys may have one");
